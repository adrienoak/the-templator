...
describe("make_create_file", () => {
  describe("with mocking filesystem", () => {
    afterEach(() => {
      mockFS.restore();
    });

    it("works with simple values without files with weird names", async () => {
    ...
    it("works with simple values", async () => {
      const { arg, base_dir, in_dir, out_dir, first_key, first_var } = setup(5);
    ...
    it("works with  nested values and variables", async () => {
      const { arg, base_dir, in_dir, out_dir, first_key, first_var } = setup();
      ...
      expect(read_content.get()).toBe(`Hello there, how are you? ${first_var}`);

      mockFS.restore();
    });

    it("doesnt call the write_file if dry_run=true", async () => {
      const { base_dir, in_dir, out_dir, arg, clean_internal_path } = setup();

      const mock = vi.fn();

      const func = make_create_file(mock);
...
describe("make_create_file_sync", () => {
  describe("with mocking filesystem", () => {
    afterEach(() => {
      mockFS.restore();
    });

    it("works with simple values without files with weird names", () => {
      const { arg, base_dir, in_dir, out_dir, clean_internal_path } = setup(
        5,
        true
      );
      ...
      const func = make_create_file_sync();
    ...
    it("doesnt call the write_file if dry_run=true", () => {
      const { base_dir, in_dir, out_dir, arg, clean_internal_path } = setup();

      const mock = vi.fn();

      const func = make_create_file_sync(mock);
